...
${technicalPlan.components.map(c => 
  `| ${c.name} | ${c.quantity || '-'} | ${c.specifications || '-'} | ${c.purpose || '-'} |`
).join('\n')}
...
${step.tips?.length ? `**💡 Tips:**\n${step.tips.map(t => `- ${t}`).join('\n')}` : ''}

${step.safetyWarnings?.length ? `**⚠️ Safety:**\n${step.safetyWarnings.map(w => `- ${w}`).join('\n')}` : ''}

${step.checkpoints?.length ? `**✓ Verify:**\n${step.checkpoints.map(c => `- [ ] ${c}`).join('\n')}` : ''}
`).join('\n')}
...
**Score:** ${item.evaluation.score}/5 ${'⭐'.repeat(item.evaluation.score)}

**Strengths:**
${item.evaluation.strengths.map(s => `- ✓ ${s}`).join('\n')}

**Areas for Improvement:**
${item.evaluation.improvements.map(s => `- → ${s}`).join('\n')}
...
export interface ExportNotesData {
  userNotes?: string;
  aiNotes?: string;
}
...
export const exportToGoogleDocs = async (
  ...
  const content = generatePlainTextContent(plan, sessionHistory, notes);
            ...
            rowData: [
              ...
              ...plan.components.map(c => ({
                values: [
                  ...
                  { userEnteredValue: { stringValue: c.quantity || '-' } },
...
const generatePlainTextContent = (
  ...
  if (isTechnical) {
    ...
    technicalPlan.components.forEach(c => {
      text += `• ${c.name} (${c.quantity || '-'}) - ${c.purpose || 'N/A'}\n`;
    });

    text += `\n---\n\nTools\n\n`;
    technicalPlan.tools.forEach(t => {
      text += `• ${t.required ? '[Required]' : '[Optional]'} ${t.name} - ${t.purpose}\n`;
    });
  ...
  // Add notes section
  if (notes && (notes.userNotes?.trim() || notes.aiNotes?.trim())) {
    text += `---\n\nPersonal Notes\n\n`;
    if (notes.userNotes?.trim()) {
      text += notes.userNotes;
      text += '\n\n';
    }
    if (notes.aiNotes?.trim()) {
